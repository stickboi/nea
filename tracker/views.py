...
from .forms import (
    ...
    UpdateDesiredPriceForm, ProfileForm, normalize_product_url,
)
...
def add_product(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            url = normalize_product_url(form.cleaned_data['product_url'])
            desired_price = form.cleaned_data.get('desired_price')

            retailer = None
            for r in Retailer.objects.filter(is_active=True):
                retailer_domain = normalize_product_url(r.base_url).split('//', 1)[-1]
                if retailer_domain in url:
                    retailer = r
                    break
            ...
            existing_pr = ProductRetailer.objects.select_related('product').filter(product_url=url).first()
            scraped = None
            ...
            if existing_pr:
                ...
                existing_pr = ProductRetailer.objects.select_related('product').filter(
                    retailer=retailer,
                    product__product_name__iexact=scraped.name.strip(),
                ).first()
                ...
                if existing_pr:
                    ...
                    product = Product.objects.create(
                        product_name=scraped.name.strip(),
                ...
                if not PriceHistory.objects.filter(product=product, retailer=retailer).exists():
            ...
            tracked_item, created = UserTrackedItem.objects.get_or_create(
                user=request.user,
                product=product,
                retailer=retailer,
            ...
            elif not tracked_item.is_active or tracked_item.desired_price != desired_price:
                tracked_item.is_active = True
                tracked_item.desired_price = desired_price
                tracked_item.save(update_fields=['is_active', 'desired_price'])
                messages.success(request, 'Product added to your tracking list!')
            else:
